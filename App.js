function App() {
  ...
  function Pad({ clip, volume, setRecording }) {
    ...
    return (
      <div onClick={playAudio} className={`btn btn-dark d-inline-table p-4 m-3 ${active && "btn-light"}`}>
    ...
    <div style={{ backgroundImage: "url('./wallpaper.jpg')", minHeight: "100vh" }} id="background-image">
      <div className="container pt-2 ">
        <div className="card shadow mb-5 rounded bg-transparent text-center text-dark border border-dark">
          ...
          <div className="card-body p-5 border border-dark">
            {audioSet.map((clip) => (
            ...
            {/* Card displaying Keycode and ID */}
            <div className="card mt-5">
              <div className="card-header">Keycode and ID</div>
              <div className="card-body">
                <ul className="list-group list-group-flush">
                  {audioSet.map((clip) => (
                    <li className="list-group-item" key={clip.id}>
                      Keycode: {clip.keyCode} | ID: {clip.id}
                    </li>
                  ))}
                </ul>
              </div>
      ...
      <footer id="footer">
        <p>
          ...
          <a href="https://github.com/Dramos02" target="_blank" id="profile-link" class="githubicon">
